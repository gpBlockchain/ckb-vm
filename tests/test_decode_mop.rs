...
pub fn test_decode_mop_adc_partial_match() {
    ...
    // Valid ADC sequence:
    let head = Rtype::new(insts::OP_ADD, 1, 2, 1);
    let next = Rtype::new(insts::OP_SLTU, 2, 1, 2);
    let neck = Rtype::new(insts::OP_ADD, 3, 1, 4);
    let body = Rtype::new(insts::OP_SLTU, 4, 3, 1);
    let tail = Rtype::new(insts::OP_OR, 2, 2, 4);
    ...
    // Test 2: Partial match - fail at next
    ...
    // Test 3: Partial match - fail at neck
    memory.store32(&(pc + 4), &next.0).unwrap();
    ...
    // Test 4: Partial match - fail at body
    memory.store32(&(pc + 8), &neck.0).unwrap();
    let body_wrong = Rtype::new(insts::OP_SLTU, 4, 3, 2);
